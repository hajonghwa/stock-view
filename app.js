...
(() => {
  ...
  function showTip(event, strong, sub) {
    tip.replaceChildren(el('strong', { class: 'num', text: strong }), sub ? el('span', { text: sub }) : null);
    tip.hidden = false;
    ...
    return el('div', { class: 'table-wrap' }, el('table', { class: 'dense' },
      ...
      el('tbody', {}, rows.map((row) => {
        ...
        return el('tr', {
          ...
          node.append(...(Array.isArray(value) ? value : [value === null || value === undefined ? '—' : value]).map(
            (v) => (v instanceof Node ? v : document.createTextNode(String(v)))));
          return node;
        }));
      }))));
  }
  ...
  function pickPanel(d, tabMarket) {
    ...
    if (S.sub === 'chart') {
      ...
      if (!hold) body.append(empty('보유 중인 종목이 아닙니다. 매도 규칙은 보유할 때만 계산합니다.'));
      else if (market === 'KR') {
  ...
  function renderKR(d) {
    ...
    out.push(card('눌림목 관찰목록', `${watch.length}종목 · 전일 종가 기준`, watch.length ? dense([
      { label: '종목', lead: true, cell: (w) => [chip(w.mode === '20일선 회복' ? '20일선' : '5일선', 'idx'), ' ', el('span', { class: 'sym', text: w.name || w.symbol })] },
